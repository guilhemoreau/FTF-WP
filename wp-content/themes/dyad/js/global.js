	...
	$( document ).ready(function(){
		...
		if ( 'none' !== $( '#colophon .widget-area .widget:first-of-type' ).css( 'float' ) ) {
			...
			$( 'a.twitter-timeline' ).each( function() {
				...
				var thisHeight = parseInt( $( this ).attr( 'height' ), 10 );
			...
			$( '.fb-page' ).each( function() {
				...
				// if the user-set height is taller than the max-height
				// make that the max-height value
				if ( $set_height > $max_height ) {
					$max_height = $set_height;
				}

				// set value of min-height depending on what settings have been used
				if ( ! $show_facepile && ! $show_posts ) {
					$min_height = 130;
				} else if ( false == $show_posts ) {
					$min_height = 220;
				} else if ( $show_facepile && $show_posts ) {
